...
\subsection*{6. Power Sets, Collections, and Families}
When denoting families of sets, collections, topologies, or power sets, it is standard practice to use a calligraphic font. You can achieve this using the \verb|\mathcal{}| command.

\begin{table}[H]
\centering
\begin{tabular}{llc}
\toprule
\textbf{Concept} & \textbf{\LaTeX{} Code} & \textbf{Rendered Output} \\
\midrule
\textbf{Power Set} & \verb|\mathcal{P}(A)| or \verb|2^A| & $\mathcal{P}(A)$ or $2^A$ \\
\textbf{Family of Sets} & \verb|\mathcal{F}| & $\mathcal{F}$ \\
\textbf{Collection of Sets} & \verb|\mathcal{C}| & $\mathcal{C}$ \\
\textbf{Topology} & \verb|\mathcal{T}| & $\mathcal{T}$ \\
\bottomrule
\end{tabular}
\end{table}
